...
def e(bts):
 ...
 for x in bts:
  o=o<<8|x;b+=8
  while b>4:b-=5;y=o>>b&31;r+=A[y]
 if b:y=o<<(5-b)&31;r+=A[y]
 return r
def d(s):
 b=o=0;r=b""
 for x in s:
 ...
 return r.hex().upper()
def ef(f):
 with open(f) as fi:b=bytes.fromhex(fi.read().strip().replace(" ","").replace("\n","").replace("-",""))
 return e(b)
def df(f):
 with open(f) as fi:s=fi.read().strip().replace(" ","").replace("\n","").replace("-","")
 return d(s)
def main():
 ...
 while True:
    ...
    if m=="q": break
    if m not in ("enc","dec"): print("Invalid"); continue
    i=input("input file: ").strip()
    if not os.path.isfile(i): print("Input DNE"); continue
